function afxSameSpace()
    ...
    afxSave(images{1},dat,V.dim,dt{1},V.mat,gz{1});
    
    for i = 2:length(images)
        dat = afxVolumeResample(images{i},XYZmm,hold{1});
        afxSave(images{i},dat,V.dim,dt{1},V.mat,gz{1});
        waitbar(i/length(images),h)
    end
end

function afxSave(fnameOrig,dat,dim,dt,mat,gz)
    ...
    afxVolumeWrite(fname,dat,dim,dt,mat,'',true);
    if gz
        gzip(fname);
        delete(fname);
    end
